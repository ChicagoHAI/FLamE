...
class WrapperConfig(object):
    ...
    def __init__(self, model_type: str, model_name_or_path: str, wrapper_type: str, task_name: str, max_seq_length: int,
                 ...
                 beta: float = 1.0, beta_requires_grad: bool = False, beta_lr: float = 2e-3, calibration_mode: str = 'multiplicative',
                 calibrate_on_step: bool = False, calibrate_on_end: bool = False):
        """
        Create a new config.
        ...
        self.calibrate_on_step = calibrate_on_step
        self.calibrate_on_end = calibrate_on_end
...
class CalibrationModule(nn.Module):
    def __init__(self, config, wrapper):
        super().__init__()
        self.mode = config.calibration_mode
        self.W = torch.ones(len(config.label_list))
        self.b = torch.zeros(len(config.label_list))
        self.wrapper = wrapper
    
    def calibrate_prediction(self, logits):
        self.W = self.W.to(logits.device)
        self.b = self.b.to(logits.device)

        return logits * self.W + self.b
    
    def update_calibrate_weights(self, calibration_batch):
        with torch.no_grad():
            mlm_labels = calibration_batch['mlm_labels']
            inputs = self.wrapper.generate_default_inputs(calibration_batch)
            outputs = self.wrapper.model(**inputs)
            batch_logits = self.wrapper.preprocessor.pvp.convert_mlm_logits_to_cls_logits(mlm_labels, outputs[0])
            
            # Looks like [batch_size, num_labels]
            # Take mean over batch

            calibration_logits = torch.mean(batch_logits, dim=0)

            if self.mode == "multiplicative":
                self.W = torch.ones_like(calibration_logits) * calibration_logits.mean() / calibration_logits
            elif self.mode == "additive":
                self.b = calibration_logits.mean() - calibration_logits
            else:
                raise ValueError("Unknown mode")
        self.log()

    def log(self):
        wandb.log({"calibration-W" : self.W, "calibration-b" : self.b})
...
class TransformerModelWrapper:
    ...
    def __init__(self, config: WrapperConfig):
        ...
        self.calibration_module = CalibrationModule(config, self)
    ...
    def from_pretrained(cls, path: str) -> 'TransformerModelWrapper':
        ...
        # load beta_requires_grad from config
        beta_requires_grad = wrapper.config.beta_requires_grad
        if beta_requires_grad:
            beta_info = torch.load('{}/beta.pt'.format(path))
            beta=beta_info.item()
        else:
            beta=1.0
        wrapper.combine_logits = AdditiveLogitsCombination(beta=beta,
                                                    beta_requires_grad=beta_requires_grad)
        wrapper.calibration_module = CalibrationModule(wrapper.config, wrapper)
        return wrapper
    ...
    def save(self, path: str) -> None:
        ...
        # save beta data in <path>/beta.pt
        if self.config.beta_requires_grad:
            torch.save(self.combine_logits.beta, '{}/beta.pt'.format(path))
    ...
    def train(self, task_train_data: List[InputExample], calibration_data, device, per_gpu_train_batch_size: int = 8, n_gpu: int = 1,
              ...
              sc_eval_during_train=False, sc_eval_steps=None, eval_data=None, calibration=False,
              sc_phe: bool = False,
              **_):
        """
        ...
        train_dataset = self._generate_dataset(task_train_data, sc_phe=sc_phe)
        train_sampler = RandomSampler(train_dataset)
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=train_batch_size)

        if calibration_data is not None:
            calibration_dataset = self._generate_dataset(calibration_data, sc_phe=sc_phe)
            calibration_dataloader = DataLoader(calibration_dataset, shuffle=False, batch_size=len(calibration_dataset))
            calibration_batch = next(iter(calibration_dataloader))
            calibration_batch = {k: t.to(device) for k, t in calibration_batch.items()}
        ...
        if lm_training or use_logits:
            ...
            unlabeled_dataset = self._generate_dataset(unlabeled_data, labelled=False, sc_phe=sc_phe)
        ...
        optimizer_grouped_parameters = [
            ...
            {'params': [p for n, p in self.model.named_parameters() if any(nd in n for nd in no_decay)],
             'weight_decay': 0.0},
            {'params': [p for p in self.combine_logits.parameters()], 'lr' : self.config.beta_lr, 'weight_decay': 0.0}
        ]
        ...
        for epoch_idx in train_iterator:
            ...
            for _, batch in enumerate(epoch_iterator):
                ...
                train_step_inputs = {
                    ...
                    'use_logits': use_logits, 'temperature': temperature, 'calibration': calibration
                }
                ...
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    if self.config.calibrate_on_step:
                        self.calibration_module.update_calibrate_weights(calibration_batch)
        ...
        if self.config.calibrate_on_end:
            self.calibration_module.update_calibrate_weights(calibration_batch)
    ...
    def eval(self, eval_data: List[InputExample], device, per_gpu_eval_batch_size: int = 8, n_gpu: int = 1,
             ...
             no_expl: bool = False, log_loss: bool = False, calibration:bool = False,
             sc_phe: bool = False) -> Dict:
        """
        ...
        eval_dataset = self._generate_dataset(eval_data, priming=priming, no_expl=no_expl, sc_phe=sc_phe)
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if logits is None:
                    logits = EVALUATION_STEP_FUNCTIONS[self.config.wrapper_type](self)(batch, calibration)
    ...
    def _generate_dataset(self, data: List[InputExample], labelled: bool = True, priming: bool = False,
                          no_expl: bool = False, sc_phe: bool = False):
        features = self._convert_examples_to_features(data, labelled=labelled, priming=priming, no_expl=no_expl, sc_phe=sc_phe)
        feature_dict = {
    ...
    def _convert_examples_to_features(self, examples: List[InputExample], labelled: bool = True,
                                      priming: bool = False, no_expl: bool = False, sc_phe: bool = False) -> List[InputFeatures]:
        features = []
        for (ex_index, example) in enumerate(examples):
            # if ex_index % 10000 == 0:
            #     logger.info("Writing example {}".format(ex_index))
            input_features = self.preprocessor.get_input_features(example, labelled=labelled, priming=priming, no_expl=no_expl, sc_phe=sc_phe)
            if self.task_helper:
    ...
    def mlm_train_step(self, labeled_batch: Dict[str, torch.Tensor],
                       ...
                       alpha: float = 0, calibration=False, **_) -> torch.Tensor:
        ...
        if 'logits' in labeled_batch and calibration:
            calibration_logits = labeled_batch['logits']
            prediction_scores = self.combine_logits(prediction_scores, calibration_logits)

        prediction_scores = self.calibration_module.calibrate_prediction(prediction_scores)
    ...
    def sequence_classifier_train_step(self, batch: Dict[str, torch.Tensor], use_logits: bool = False,
                                       temperature: float = 1, calibration=False, **_) -> torch.Tensor:
        ...
        if 'logits' in batch and calibration:
            calibration_logits = batch['logits']
            prediction_scores = self.combine_logits(prediction_scores, calibration_logits)
            loss = nn.functional.cross_entropy(prediction_scores, inputs['labels'])

        if use_logits:
            logits_predicted, logits_target = outputs[0], batch['logits']
            return distillation_loss(logits_predicted, logits_target, temperature)
        else:
            wandb.log({"sc_train_loss" : loss})
            return loss

    def mlm_eval_step(self, batch: Dict[str, torch.Tensor], calibration=False) -> torch.Tensor:
    ...
    def plm_eval_step(self, batch: Dict[str, torch.Tensor], calibration=False) -> torch.Tensor:
    ...
    def sequence_classifier_eval_step(self, batch: Dict[str, torch.Tensor], calibration=False) -> torch.Tensor:
        """Perform a sequence classifier evaluation step."""
        inputs = self.generate_default_inputs(batch)
        prediction_scores = self.model(**inputs)[0]
        if 'logits' in batch and calibration:
            calibration_logits = batch['logits']
            prediction_scores = self.combine_logits(prediction_scores, calibration_logits)
