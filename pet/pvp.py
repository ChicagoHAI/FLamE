...
class EsnliPVP(PVP):
    ...
    def get_parts(self, example: InputExample, no_expl=False) -> FilledPattern:
        text_a = self.shortenable(self.remove_final_punc(example.text_a))
        text_b = self.shortenable(example.text_b)
        expl = self.shortenable(example.expl) if example.expl != None else None

        # lowercase first character of hypothesis & expl.
        text_b = self.lowercase_first(text_b)

        if expl != None and not no_expl:
            expl = self.lowercase_first(expl)
            if self.pattern_id == 0 or self.pattern_id == 2:
                return ['"', text_a, '" ?'], [self.mask, ', "', text_b, '" because "', expl, '"']
            elif self.pattern_id == 1 or self.pattern_id == 3:
                return [text_a, '?'], [self.mask, ',', text_b, ' because ', expl]
            elif self.pattern_id == 4 or self.pattern_id == 6:
                # p and h. Is it possible? [MASK], because expl
                return [text_a, ' and ', text_b, '. Is it possible?'], [self.mask, ', because ', expl]
            elif self.pattern_id == 5 or self.pattern_id == 7:
                # p. Since expl, h is [MASK].
                return [text_a, '.'], ['Since ', expl, ', ', text_b, ' is ', self.mask]
        else:
            if self.pattern_id == 0 or self.pattern_id == 2:
                return ['"', text_a, '" ?'], [self.mask, ', "', text_b, '"']
            elif self.pattern_id == 1 or self.pattern_id == 3:
                ...
                # p and h. Is it possible? [MASK]
                return [text_a, ' and ', text_b, '. Is it possible?'], [self.mask]
            elif self.pattern_id == 5 or self.pattern_id == 7:
                # p. h is [MASK].
                return [text_a, '.'], [text_b, ' is ', self.mask]


    def verbalize(self, label) -> List[str]:

        if self.pattern_id in (0, 1, 5, 6):
            return EsnliPVP.VERBALIZER_A[label]
        return EsnliPVP.VERBALIZER_B[label]
        
class AnalyticEntailmentPVP(PVP):
    VERBALIZER_A = {
        "entailment": ["Right"],
        "neutral": ["Maybe"]
    }
    ...
    def get_parts(self, example: InputExample, no_expl=False) -> FilledPattern:
        ...
        expl = self.lowercase_first(self.shortenable(example.expl)) if example.expl != None else None
        text = self.remove_final_punc(text)

        if expl != None and not no_expl:
            if self.pattern_id in (0, 1):
                return [text, "?"], ['Since ', expl, ', ', self.mask]
            if self.pattern_id in (2, 3):
                return [text, "?"], ['Answer: ', self.mask, ' because ', expl]
            if self.pattern_id in (4, 5):
                return [text, " is ", self.mask], ['because ', expl]
        else:
            if self.pattern_id in (0, 1):
                return [text, "?"], [self.mask]
            if self.pattern_id in (2, 3):
                return [text, "?"], ['Answer: ', self.mask]
            if self.pattern_id in (4, 5):
                return [text, " is ", self.mask], []

        raise Exception

    def verbalize(self, label) -> List[str]:

        if self.pattern_id % 2 == 0:
            return AnalyticEntailmentPVP.VERBALIZER_A[label]
        return AnalyticEntailmentPVP.VERBALIZER_B[label]

class FantasyReasoningPVP(PVP):
    VERBALIZER_A = {
        "Yes": ["Yes"],
        "No": ["No"]
    }
    ...
    def verbalize(self, label) -> List[str]:
        ...
        if self.pattern_id % 2 == 0:
            return FantasyReasoningPVP.VERBALIZER_A[label]
        return FantasyReasoningPVP.VERBALIZER_B[label]
  

class IronyIdentificationPVP(PVP):
    VERBALIZER_A = {
        "ironic": ["Yes"],
        "not ironic": ["No"]
    }
    ...
    def get_parts(self, example: InputExample, no_expl=False) -> FilledPattern:
        ...
        if expl != None and not no_expl:
            if self.pattern_id in (0, 1):
                return [text, "Is the statement ironic?"], ['Since ', expl, ', ', self.mask]
            if self.pattern_id in (2, 3):
                return ["Is this statement ironic? Statement: ", text], [self.mask, ', because ', expl]
            if self.pattern_id in (4, 5):
                return [text, "Is there an irony in the claim?"], ['Since ', expl, ', ', self.mask]
            if self.pattern_id in (6, 7):
                return ["Is there an irony in this claim? Claim: ", text], [self.mask, ', because ', expl]

        else:
            if self.pattern_id in (0, 1):
                return [text, "Is the statement ironic?"], [self.mask]
            if self.pattern_id in (2, 3):
                return ["Is this statement ironic? Statement: ", text], [self.mask]
            if self.pattern_id in (4, 5):
                return [text, "Is there an irony in the claim?"], [self.mask]
            if self.pattern_id in (6, 7):
                return ["Is there an irony in this claim? Claim: ", text], [self.mask]
    ...
    def verbalize(self, label) -> List[str]:
        ...
        if self.pattern_id % 2 == 0:
            return IronyIdentificationPVP.VERBALIZER_A[label]
        return IronyIdentificationPVP.VERBALIZER_B[label]
...
PVPS = {
    ...
    'esnli': EsnliPVP,
    'esnli-100':EsnliPVP,
    'esnli-100-100':EsnliPVP,
    'esnli-100-1000':EsnliPVP,
    'esnli-dev-100':EsnliPVP,
    'analytical-entailment': AnalyticEntailmentPVP,
    'fantasy-reasoning': FantasyReasoningPVP,
    'irony-identification': IronyIdentificationPVP,
