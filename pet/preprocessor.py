...
class Preprocessor(ABC):
    ...
    def __init__(self, wrapper, task_name, pattern_id: int = 0, verbalizer_file: str = None, sc_phe: bool = False):
        """
...
class SequenceClassifierPreprocessor(Preprocessor):
    ...
    def get_input_features(self, example: InputExample, sc_phe=False, **kwargs) -> InputFeatures:
        ...
        if inputs is None:
            if sc_phe:
                inputs = self.wrapper.tokenizer.encode_plus(
                    example.text_a,
                    example.text_b + example.expl if example.text_b and example.expl else example.text_b,
