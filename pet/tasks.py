...
class EsnliJSONLProcessor(DataProcessor):
    def get_train_examples(self, data_dir, no_expl=True, expl_file=None, shuffle_expl=False, concat_expl=False,
                           with_three_expl=False, calibration=False, train_gold_expl=False,
                           train_gold_gen_expl=False, templated_explanation=False):
        return self._create_examples(
            os.path.join(data_dir, "train.jsonl"), "train", no_expl=no_expl,
            expl_file=expl_file, shuffle_expl=shuffle_expl, concat_expl=concat_expl,
            with_three_expl=with_three_expl, calibration=calibration, gold_expl=train_gold_expl,
            gold_gen_expl=train_gold_gen_expl,
            templated_explanation=templated_explanation
        )

    def get_dev_examples(self, data_dir, no_expl=True, expl_file=None, shuffle_expl=False, concat_expl=False,
                         with_three_expl=False, calibration=False, test_gold_expl=False):
        return self._create_examples(
            os.path.join(data_dir, "dev.jsonl"), "dev", no_expl=no_expl,
            expl_file=expl_file, shuffle_expl=shuffle_expl, concat_expl=concat_expl,
            with_three_expl=with_three_expl, calibration=calibration, gold_expl=test_gold_expl
        )

    def get_test_examples(self, data_dir, no_expl=True, expl_file=None, shuffle_expl=False, concat_expl=False, 
                          with_three_expl=False, calibration=False, test_gold_expl=False):
        return self._create_examples(
            os.path.join(data_dir, "test.jsonl"), "test", no_expl=no_expl,
            expl_file=expl_file, shuffle_expl=shuffle_expl, concat_expl=concat_expl,
            with_three_expl=with_three_expl, calibration=calibration, gold_expl=test_gold_expl
        )

    def get_unlabeled_examples(self, data_dir, no_expl=True, expl_file=None, shuffle_expl=False, concat_expl=False) -> List[InputExample]:
        return self.get_train_examples(data_dir, no_expl=no_expl, expl_file=expl_file, shuffle_expl=shuffle_expl, concat_expl=concat_expl)

    def get_calibration_examples(self):
        phe_pairs = [
            ("", "", None),
            ("N/A", "N/A", None),
            ("N/A", "N/A", "N/A"),
            ("N/A", "N/A", "Yes N/A No N/A Maybe N/A"),
            ("", "", "Yes No Maybe")
        ]
        examples = [
            InputExample(f"calibration-{i}", p, text_b=h, expl=e) for i, (p, h, e) in enumerate(phe_pairs)
        ]
        return examples
    ...
    @staticmethod
    def _add_to_examples(examples, guid, premise, hypothesis, label, expl, logits):
        example = InputExample(
            guid=guid,
            text_a=premise,
            text_b=hypothesis,
    ...
    @staticmethod
    def _create_examples(path: str, set_type: str, no_expl=True, expl_file=None,
     shuffle_expl=False, concat_expl=False, with_three_expl=False, calibration=False,
     gold_expl=False, gold_gen_expl=False, templated_explanation=False) -> List[InputExample]:
        '''
        with_three_expl: Use all three explanations, one at a time. In other words, the explanations are not concatenated.
        '''
        add_generated_expl = expl_file is not None
        if add_generated_expl: no_expl = False

        logits_path = EsnliJSONLProcessor._get_logits_path_from_file_path(path)
        add_logits = os.path.exists(logits_path) and not no_expl

        if with_three_expl and not add_generated_expl:
            raise Exception("Must have access to (GPT) generated explanations to evaluate with them.")

        if add_logits:
            logits_iter = map(lambda row: [float(f) for f in row.split()], open(logits_path))
        else:
        ...
        example_iter = pd.read_json(path, orient='records', lines=True).iterrows()
        examples = []
        for i, ((_, line), logits, (_, generated_expls)) in enumerate(zip(example_iter, logits_iter, generated_expl_iter)):
            ...
            if no_expl: # ph
                EsnliJSONLProcessor._add_to_examples(examples, guid, premise, hypothesis, label, None, logits)
            # phe customize
            if with_three_expl: # phe (contains all generated explanations)
                for j in range(len(EsnliJSONLProcessor.get_labels())): # with all three generated explanations
                    EsnliJSONLProcessor._add_to_examples(examples, guid, premise, hypothesis, label, generated_expls[j], logits)
            if gold_expl:
                if templated_explanation:
                    EsnliJSONLProcessor._add_to_examples(examples, guid, premise, hypothesis, label, line['templated_explanation'], logits)
                else:
                    EsnliJSONLProcessor._add_to_examples(examples, guid, premise, hypothesis, label, line[4], logits)
            if gold_gen_expl:
                gold_label_index = EsnliJSONLProcessor.get_labels().index(label)
                EsnliJSONLProcessor._add_to_examples(examples, guid, premise, hypothesis, label, generated_expls[gold_label_index], logits)

        return examples


class EhansProcessor(EsnliJSONLProcessor):
    @staticmethod
    def get_labels():
        return ["entailment", "neutral"]

    def get_additional_test_examples(self, data_dir, no_expl=True, expl_file=None, shuffle_expl=False, concat_expl=False, 
                          with_three_expl=False, calibration=False, test_gold_expl=False):
        return self._create_examples(
            os.path.join(data_dir, "test_ood.jsonl"), "additional_test", no_expl=no_expl,
            expl_file=expl_file, shuffle_expl=shuffle_expl, concat_expl=concat_expl,
            with_three_expl=with_three_expl, calibration=calibration, gold_expl=test_gold_expl
        )
    ...
    def _create_examples(path: str, set_type: str, no_expl=True, expl_file=None,
     ...
     gold_expl=False, gold_gen_expl=False, templated_explanation=False) -> List[InputExample]:
        '''
        with_three_expl: Use all three explanations (two explanations for ehans), one at a time. In other words, the explanations are not concatenated.
        '''
        add_generated_expl = expl_file is not None
        if add_generated_expl: no_expl = False

        logits_path = EhansProcessor._get_logits_path_from_file_path(path)
        add_logits = os.path.exists(logits_path) and not no_expl
        ...
        for i, ((_, line), logits, (_, generated_expls)) in enumerate(zip(example_iter, logits_iter, generated_expl_iter)):
            ...
            premise = line['premise']
            hypothesis = line['hypothesis']
            labels = EhansProcessor.get_labels()

            if no_expl: # ph
                EsnliJSONLProcessor._add_to_examples(examples, guid, premise, hypothesis, label, None, logits)
            # phe customize
            if with_three_expl: # phe (contains all generated explanations)
                for j in range(len(labels)): # with all two generated explanations
                    EsnliJSONLProcessor._add_to_examples(examples, guid, premise, hypothesis, label, generated_expls[j], logits)
            if gold_expl:
                EsnliJSONLProcessor._add_to_examples(examples, guid, premise, hypothesis, label, line[4], logits)
            if gold_gen_expl:
                gold_label_index = labels.index(label)
                EsnliJSONLProcessor._add_to_examples(examples, guid, premise, hypothesis, label, generated_expls[gold_label_index], logits)

        return examples


class BigBenchProcessor(EsnliJSONLProcessor):

    def get_dev_examples(self, data_dir, no_expl=True, expl_file=None, shuffle_expl=False, concat_expl=False,
                         with_three_expl=False, calibration=False):
        return self._create_examples(
            os.path.join(data_dir, "dev+test.jsonl"), "dev", no_expl=no_expl,
            expl_file=expl_file, shuffle_expl=shuffle_expl, concat_expl=concat_expl,
            with_three_expl=with_three_expl, calibration=calibration
        )

    def get_test_examples(self, data_dir, no_expl=True, expl_file=None, shuffle_expl=False, concat_expl=False, 
                          with_three_expl=False, calibration=False):
        return self._create_examples(
            os.path.join(data_dir, "dev+test.jsonl"), "test", no_expl=no_expl,
            expl_file=expl_file, shuffle_expl=shuffle_expl, concat_expl=concat_expl,
            with_three_expl=with_three_expl, calibration=calibration
        )

    @staticmethod
    def _add_to_examples(examples, input, label, expl, logits):
        example = InputExample(
            guid="",
            text_a=input,
            label=label,
    ...
    @staticmethod
    def _create_examples(path: str, set_type: str, no_expl=True, expl_file=None, shuffle_expl=False, concat_expl=False, 
    with_three_expl=False, calibration=False, gold_expl=False, gold_gen_expl=False) -> List[InputExample]:
        pass


class AnalyticalEntailmentProcessor(BigBenchProcessor):
    @staticmethod
    def get_labels():
        return ['entailment', 'neutral']
    ...
    with_three_expl=False, calibration=False, gold_expl=False, gold_gen_expl=False) -> List[InputExample]:
        ...
        logits_path = BigBenchProcessor._get_logits_path_from_file_path(path)
        add_logits = os.path.exists(logits_path) and not no_expl
        ...
        for i, ((_, line), logits, (_, generated_expls)) in enumerate(zip(example_iter, logits_iter, generated_expl_iter)):
            ...
            label = line['label']
            input = line['input']
            labels = AnalyticalEntailmentProcessor.get_labels()
            num_labels = len(labels)

            if no_expl: # ph
                BigBenchProcessor._add_to_examples(examples, input, label, None, logits)
            # phe customize
            if with_three_expl: # phe (contains all generated explanations)
                for j in range(num_labels): # with all two generated explanations
                    BigBenchProcessor._add_to_examples(examples, input, label, generated_expls[j], logits)
            if gold_expl: 
                BigBenchProcessor._add_to_examples(examples, input, label, line[2], logits)
            if gold_gen_expl: # potentially also with gold explanation
                gold_label_index = labels.index(label)
                BigBenchProcessor._add_to_examples(examples, input, label, generated_expls[gold_label_index], logits)
                
        return examples


class FantasyReasoningProcessor(BigBenchProcessor):
    @staticmethod
    def get_labels():
        return ['Yes', 'No']
    ...
    with_three_expl=False, calibration=False, gold_expl=False, gold_gen_expl=False) -> List[InputExample]:
        ...
        for i, ((_, line), logits, (_, generated_expls)) in enumerate(zip(example_iter, logits_iter, generated_expl_iter)):
            ...
            label = line['label']
            input = line['input']
            labels = FantasyReasoningProcessor.get_labels()
            num_labels = len(labels)
...
class IronyIdentificationProcessor(BigBenchProcessor):
    @staticmethod
    def get_labels():
        return ['ironic', 'not ironic']
    ...
    with_three_expl=False, calibration=False, gold_expl=False, gold_gen_expl=False) -> List[InputExample]:
        ...
        for i, ((_, line), logits, (_, generated_expls)) in enumerate(zip(example_iter, logits_iter, generated_expl_iter)):
            ...
            label = line['label']
            input = line['input']
            labels = IronyIdentificationProcessor.get_labels()
            num_labels = len(labels)
...
PROCESSORS = {
    ...
    "esnli": EsnliJSONLProcessor,
    "analytical-entailment": AnalyticalEntailmentProcessor,
    'fantasy-reasoning': FantasyReasoningProcessor,
    'irony-identification': IronyIdentificationProcessor,
...
ADDITIONAL_TEST_SET = "additional_test"

SET_TYPES = [TRAIN_SET, DEV_SET, TEST_SET, UNLABELED_SET, ADDITIONAL_TEST_SET]
...
def load_examples(
    ...
    *_,
    ...
    seed: int = 42,
    no_expl: bool = True,
    expl_file=None, concat_expl=False, shuffle_expl=False,
    eval_with_three_labels_explanations_logits=False,
    train_with_three_labels_explanations=False,
    calibration=False,
    train_gold_expl=False,
    train_gold_gen_expl=False,
    test_gold_expl=False,
    templated_explanation=False
    ) -> List[InputExample]:
    ...
    if set_type == DEV_SET:
        ...
        concat_expl=concat_expl, shuffle_expl=shuffle_expl, 
        with_three_expl=eval_with_three_labels_explanations_logits,
        calibration=calibration,
        test_gold_expl=test_gold_expl
        )
        ...
        concat_expl=concat_expl, shuffle_expl=shuffle_expl, 
        with_three_expl=train_with_three_labels_explanations,
        calibration=calibration,
        train_gold_expl=train_gold_expl,
        train_gold_gen_expl=train_gold_gen_expl,
        templated_explanation=templated_explanation
        )
    elif set_type == UNLABELED_SET:
        examples = processor.get_unlabeled_examples(data_dir, no_expl=no_expl, expl_file=expl_file,
        concat_expl=concat_expl, shuffle_expl=shuffle_expl)
        for example in examples:
            example.label = processor.get_labels()[0]
    elif set_type == ADDITIONAL_TEST_SET:
        examples = processor.get_additional_test_examples(data_dir, no_expl=no_expl, expl_file=expl_file,
        concat_expl=concat_expl, shuffle_expl=shuffle_expl, 
        with_three_expl=eval_with_three_labels_explanations_logits,
        calibration=calibration,
        test_gold_expl=test_gold_expl
    ...
    if eval_with_three_labels_explanations_logits and set_type in [DEV_SET, TEST_SET, ADDITIONAL_TEST_SET]:
        # do not shuffle when we evalute with explanations for all labels
        assert num_examples != None
        if 0 < num_examples < len(examples):
            examples = examples[:num_examples]
